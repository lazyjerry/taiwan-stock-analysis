...
description: 當使用者要把台股財報分析接到股價、計算合理價、拆成悲觀/中性/樂觀三種價格區間、或輸出 1-5 分估值評級與分批建議時使用。優先讀取 `*_analysis.json`，並用 `scripts/build_valuation_report.py` 產出可直接貼給使用者的估值表。
---
...
本 repo 目前找不到 `analyze_tw_stock.py`。若要確認相對位置，現有最接近的財報抓取腳本是 `../taiwan-stock-analysis/scripts/fetch_goodinfo.py`，但它輸出的是 `*_raw_data.json`，不能直接當成這個 skill 的 `*_analysis.json` 來源。

## Quick Start

1. 先和使用者確認股票代號、公司名稱、`*_analysis.json` 檔名是否一致
2. 只有確認同一個標的後，才使用 `*_analysis.json`
3. 用 `scripts/build_valuation_report.py` 產出估值報告
4. 若使用者提供現價，帶入 `--current-price`
5. 若使用者明確指定悲觀/中性/樂觀 EPS，改用覆寫參數
6. 若只有股票代號、沒有 `*_analysis.json`，先停下來確認資料來源，不要直接假設 repo 內有 `analyze_tw_stock.py`
...
- 只有股票代碼
  ...
  但它只會產出 `*_raw_data.json`，所以要先請使用者確認是否改走財報分析 skill，或直接提供 `*_analysis.json`
- 只有口頭假設
  直接用自訂 EPS 參數出估值表
