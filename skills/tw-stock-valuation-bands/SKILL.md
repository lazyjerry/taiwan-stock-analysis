...
- 預設目標：Obsidian 當前開啟的檔案（腳本讀 `--vault-root/.obsidian/workspace.json` 的 `active` leaf，**不是** `lastOpenFiles[0]`）；偵測不到才建新筆記 `<公司>_<代碼>_valuation.md`
- 指定筆記路徑：帶 `--note-path <相對 vault 路徑>`
- 寫入模式：`--write-mode overwrite`（複寫，預設）｜ `--write-mode append`（附加到檔尾，去重 frontmatter）
- Vault 根目錄：`--vault-root <dir>`，預設當前目錄 `.`

**若當前開啟檔已有內容，先問使用者複寫或附加**，再傳對應 `--write-mode`。原因：腳本非互動時遇非空檔預設複寫、不會停下來問；詢問由代理在呼叫前完成，以免覆蓋既有內容。
