...
- 預設目標：Obsidian 當前開啟的檔案（腳本讀 `--vault-root/.obsidian/workspace.json` 的 `active` leaf，**不是** `lastOpenFiles[0]`）；偵測不到才建新筆記 `<代碼> <名稱>.md`（如 `2330 台積電.md`）
- 指定筆記路徑：帶 `--note-path <相對 vault 路徑>`
- 寫入模式：`--write-mode overwrite`（複寫，預設）｜ `append`（附加到檔尾，去重 frontmatter）｜ `newfile`（不動既有檔，改用 `<代碼> <名稱>.md` 建立不重複的新筆記，自動改名）
